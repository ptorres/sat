            ...
            foreach ($Complemento->childNodes as $node) {
                if ($node->nodeType == XML_ELEMENT_NODE) {
                    if ($node->nodeName == "tfd:TimbreFiscalDigital" ||
                        ...
                        $this->status = "CCE155 El nodo cce11:ComercioExterior solo puede coexistir con los complementos Timbre Fiscal Digital, otros derechos e impuestos, leyendas fiscales, recepción de pago, CFDI registro fiscal.".$node->nodeName;
                        $this->codigo .= "; CCE155";
                        $error=true;
                    }
                }
        ...
        if ($cce_Emisores->length == 0) {
            ...
            if ($Domicilios->length > 0) {
                ...
                if ($localidad != "") {
                    $ok = $this->Checa_Catalogo("c_Localidad", $localidad, $estado);
                    if (!$ok) {
                        $this->status = "CCE171 El atributo cce11:ComercioExterior:Emisor:Domicilio:Localidad debe contener una clave del catálogo de catCFDI:c_Localidad donde la columna clave de c_Estado debe ser igual a la clave registrada en el atributo Estado.";
                        $this->codigo .= "; CCE171";
                        $error=true;
                    }
                }
